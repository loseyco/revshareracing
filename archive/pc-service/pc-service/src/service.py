...
from core import device, telemetry, laps, controls, command_queue, graphics, updater, joystick_monitor
...
class RigService:
    ...
    def _setup_joystick_monitor(self):
        """Setup joystick input monitoring for conditional action blocking"""
        try:
            ...
            # Register action check callbacks
            def can_enter_car() -> bool:
                """Check if enter_car action is allowed"""
                # Allow if there's an active timed session
                if self.timed_session_state and self.timed_session_state.get('active'):
                    return True
                # TODO: Add credit/payment check here
                # For now, allow if no timed session (backwards compatible)
                return True
            
            def can_reset_car() -> bool:
                """Check if reset_car action is allowed"""
                # Always allow reset during timed session (they can reset wrecked car)
                if self.timed_session_state and self.timed_session_state.get('active'):
                    return True
                # If no timed session, allow reset (backwards compatible)
                # TODO: Could block reset if no payment/credits
                return True
            
            self.joystick_monitor.register_action_check("enter_car", can_enter_car)
            self.joystick_monitor.register_action_check("reset_car", can_reset_car)
            
            print("[JOYSTICK] Monitor setup complete")
        except Exception as e:
    ...
    def start(self):
        ...
        # Start joystick monitor if available
        if self.joystick_monitor:
            try:
                self.joystick_monitor.set_enabled(True)
            except Exception as exc:
                print(f"[WARN] Failed to enable joystick monitor: {exc}")
