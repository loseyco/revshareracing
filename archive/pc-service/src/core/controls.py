...
class ControlsManager:
    ...
    def execute_combo(self, combo: Optional[str], source: str = "manual") -> bool:
        ...
        # Check if this is a joystick/steering wheel button binding
        if combo.startswith("Button") or combo.startswith("button"):
            self._log_action("combo", combo, source, {
                'success': False, 
                'message': f'Cannot execute joystick button binding ({combo}). Only keyboard bindings can be executed programmatically.'
            })
            return False
    ...
    def execute_action(self, action: str, source: str = "manual") -> Dict[str, str]:
        ...
        # Check if this is a joystick/steering wheel button binding
        # These cannot be executed programmatically - only keyboard bindings work
        if combo.startswith("Button") or combo.startswith("button"):
            action_label = binding.get("label", action)
            result = {
                'success': False, 
                'message': f'Cannot execute {action_label}: This control is bound to a steering wheel/joystick button ({combo}). Only keyboard bindings can be executed programmatically. Please configure this action to use a keyboard key in iRacing settings.'
            }
            self._log_action(action, combo, source, result)
            return result
